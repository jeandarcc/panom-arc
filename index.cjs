...
const ARC_ANCHOR_MASK = 0x9009;
const ARC_ANCHOR_ON = 0x100d;
const ARC_PAYLOAD_MASK = (~ARC_ANCHOR_MASK) & 0xffff;
const ARC_PAYLOAD_INDICES = [1, 2, 4, 5, 6, 7, 8, 9, 10, 11, 13, 14];
const DEFAULT_WIDTH = 320;
const DEFAULT_HEIGHT = 240;

function applyArcAnchors(bits) {
  return (bits & ARC_PAYLOAD_MASK) | ARC_ANCHOR_ON;
}

function payloadBits(bits) {
...
function isArcCellOn(bits, bitIndex) {
  return (
    bitIndex === 0 ||
    bitIndex === 3 ||
    bitIndex === 12 ||
...
function renderArcToPixels(bits, scenario, options) {
...
function makeArcFixtures(count = 20, seed = 0xdeadbeef) {
  const fixtures = [
...
function makeArcScenarios(count, options) {
  const width = options?.width ?? DEFAULT_WIDTH;
  const height = options?.height ?? DEFAULT_HEIGHT;
...
function preprocessArcFrameFromRgba(data, options) {
...
function detectArcPatternFromRgba(data, options) {
